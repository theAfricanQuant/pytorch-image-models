...
class EfficientNetBuilder:
    ...
    def _make_block(self, ba, block_idx, block_count):
        ...
        if bt == 'ir':
            ...
            if self.verbose:
                logging.info(f'  InvertedResidual {block_idx}, Args: {str(ba)}')
            ...
            ba['se_kwargs'] = self.se_kwargs
            if self.verbose:
                logging.info(f'  DepthwiseSeparable {block_idx}, Args: {str(ba)}')
            block = DepthwiseSeparableConv(**ba)
        elif bt == 'er':
            ba['drop_connect_rate'] = drop_connect_rate
            ba['se_kwargs'] = self.se_kwargs
            if self.verbose:
                logging.info(f'  EdgeResidual {block_idx}, Args: {str(ba)}')
            block = EdgeResidual(**ba)
        elif bt == 'cn':
            if self.verbose:
                logging.info(f'  ConvBnAct {block_idx}, Args: {str(ba)}')
            block = ConvBnAct(**ba)
        else:
    ...
    def __call__(self, in_chs, model_block_args):
        ...
        for stage_idx, stage_block_args in enumerate(model_block_args):
            ...
            if self.verbose:
                logging.info(f'Stack: {stage_idx}')
            ...
            for block_idx, block_args in enumerate(stage_block_args):
                ...
                do_extract = False
                if self.feature_location == 'post_exp':
                    if block_args['stride'] > 1 or (last_stack and last_block) :
                        do_extract = True
                elif self.feature_location == 'pre_pwl':
                    if last_block:
                        next_stage_idx = stage_idx + 1
                        do_extract = (
                            True
                            if next_stage_idx >= len(model_block_args)
                            else model_block_args[next_stage_idx][0]['stride'] > 1
                        )
                if do_extract:
                ...
                if extract_features:
                    ...
                    if feature_module:
                        feature_module = f'blocks.{stage_idx}.{block_idx}.{feature_module}'
